...
object VoiceCommandParser {
     ...
     *
     * @param rawText Spoken text recognized by SpeechRecognizer.
     * @param requireWakeWord If true, the text must start with or contain a wake word;
     *                        if false, direct commands like "play Starboy" or "pause" are accepted.
     */
    fun parse(rawText: String, requireWakeWord: Boolean = true): VoiceCommand {
        ...
        // If the command body is empty after stripping wake word (e.g. user just said "Hi AirBeats")
        if (commandBody.isEmpty()) {
            return VoiceCommand.Unknown(rawText)
        }

        return parseCommandBody(commandBody, rawText)
    }

    private fun parseCommandBody(body: String, rawOriginal: String): VoiceCommand {
        val normalized = body.trim()

        // 1. Play / Search Song
        val playPrefixMatch = Regex(
            "^(?:play|listen\\s+to|put\\s+on)(?:\\s+the\\s+song|\\s+the\\s+track|\\s+song|\\s+track)?(?:\\s+called)?(?:\\s+)(.+)$",
            RegexOption.IGNORE_CASE
        ).find(normalized)
        ...
        // 2. Simple Resume / Play without query
        if (normalized == "play" || normalized == "resume" || normalized == "continue" ||
            normalized == "unpause" || normalized == "play music" || normalized == "start music"
        ) {
        ...
        if (volumeMatch != null) {
            val level = volumeMatch.groupValues[1].toIntOrNull()
            if (level != null) {
