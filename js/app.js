document.addEventListener('DOMContentLoaded', () => {
    ...
    // Previous Versions Popup Handler (Works for both Android & Windows)
    const prevVersionsBtn = document.getElementById('prevVersionsBtn');
    const previousVersionsModal = document.getElementById('previousVersionsModal');
    const modalPlatformTabs = document.getElementById('modalPlatformTabs');

    function renderPreviousVersionsPopup(platform = currentPlatform) {
        const popupList = document.getElementById('previousVersionsList');
        const popupTitle = document.getElementById('prevModalTitle');
        const popupSubtitle = document.getElementById('prevModalSubtitle');
        if (!popupList) return;

        if (modalPlatformTabs) {
            modalPlatformTabs.querySelectorAll('.platform-tab').forEach(b => {
                b.classList.toggle('active', b.dataset.modalPlatform === platform);
            });
        }
        ...
        if (platform === 'windows') {
            filteredReleases = allReleases.filter(rel => rel.assets && rel.assets.some(a => /\.exe$/i.test(a.name)));
            if (popupTitle) popupTitle.innerHTML = `<i class="bi bi-windows" style="color: var(--accent);"></i> Previous Windows Builds (.exe)`;
            if (popupSubtitle) popupSubtitle.textContent = `Showing all ${filteredReleases.length} previous Windows desktop executable releases`;
        } else {
            filteredReleases = allReleases.filter(rel => rel.assets && rel.assets.some(a => /\.apk$/i.test(a.name)));
            if (popupTitle) popupTitle.innerHTML = `<i class="bi bi-android2" style="color: var(--primary);"></i> Previous Android Releases (.apk)`;
            if (popupSubtitle) popupSubtitle.textContent = `Showing all ${filteredReleases.length} previous Android signed APK releases`;
        }

        if (!filteredReleases || filteredReleases.length === 0) {
            popupList.innerHTML = `<div class="note" style="text-align: center; padding: 2rem 0;">No ${platform} releases found in repository history.</div>`;
            return;
        }
        ...
        filteredReleases.forEach(rel => {
            ...
            downloadableAssets.forEach(asset => {
                ...
                const icon = platform === 'windows' ? (isSetup ? 'bi-windows' : 'bi-box-seam') : 'bi-android2';

                assetButtons += `
                    <a href="${asset.browser_download_url}" class="download-sm-btn" target="_blank" rel="noopener noreferrer" style="margin-top: 0.3rem;">
            ...
            html += `
                <div class="version-compact-item" style="margin-bottom: 0.8rem; background: var(--surface-light); border-radius: 14px; padding: 1rem 1.2rem; border-left: 4px solid ${platform === 'windows' ? 'var(--accent)' : 'var(--primary)'};">
                    <div class="version-compact-header" style="display: flex; justify-content: space-between; align-items: center;">
                        <span class="version-tag-sm" style="background: var(--gradient); padding: 0.2rem 0.7rem; border-radius: 20px; font-weight: 700; color: white; font-size: 0.85rem;">${versionTag}</span>
                        <span class="version-date-sm" style="font-size: 0.8rem; color: var(--text-muted);"><i class="bi bi-calendar3"></i> ${pubDate}</span>
                    </div>
                    <div style="font-size: 0.85rem; color: var(--text-muted); margin: 0.5rem 0;">✨ ${cleanNotes}</div>
                    <div class="version-download-sm" style="display: flex; flex-wrap: wrap; gap: 0.5rem; justify-content: flex-end;">
                        ${assetButtons}
                    </div>
                </div>
            `;
    ...
    if (prevVersionsBtn && previousVersionsModal) {
        prevVersionsBtn.addEventListener('click', () => {
            renderPreviousVersionsPopup(currentPlatform);
            previousVersionsModal.classList.add('show');
        });
    }

    if (modalPlatformTabs) {
        modalPlatformTabs.querySelectorAll('.platform-tab').forEach(btn => {
            btn.addEventListener('click', () => {
                renderPreviousVersionsPopup(btn.dataset.modalPlatform);
            });
        });
    }
